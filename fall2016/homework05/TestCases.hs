...
formatTests ts = (intercalate "\n" $ map formatTest ts) ++ "\n"

formatTest :: (String, [Bool]) -> String
formatTest (name, res)
  | and res = name ++ ": " ++ show passed ++ "/" ++ show total ++ " successful tests."
  | or res  = name ++ ": " ++ show passed ++ "/" ++ show total ++ " successful tests. " ++
              "Failing tests: " ++ intercalate ", " (map show (findFailing res))
  where
    passed = length $ filter (== True) res
    total = length res

findFailing :: [Bool] -> [Integer]
findFailing = findFailing' 1
  where
    findFailing' _ []         = []
    findFailing' n (False:xs) = n : findFailing' (n+1) xs
    findFailing' n (True:xs)  =     findFailing' (n+1) xs
